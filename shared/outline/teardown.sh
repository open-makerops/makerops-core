#!/bin/bash
# Tears down the Outline stack completely.
...
# Note: SECRET_KEY and UTILS_SECRET in .env are not reset — replace them with
#       OVERWRITE_ME beforehand if you want a fully fresh install on the next start.
set -e
...
echo ""
echo "Done. To reinstall completely fresh (new secrets):"
echo "  sed -i 's/^SECRET_KEY=.*/SECRET_KEY=OVERWRITE_ME/' .env"
echo "  sed -i 's/^UTILS_SECRET=.*/UTILS_SECRET=OVERWRITE_ME/' .env"
echo "  sed -i 's/^DB_PASSWORD=.*/DB_PASSWORD=OVERWRITE_ME/' .env"
echo "  ./start.sh"
